...
class LstmLstm(chainer.Chain):

	"""docstring for LstmLstm"""
	def __init__(self, n_voc_src, n_voc_trg, n_units, gpu, word_idx_trg):
		super(LstmLstm, self).__init__(
			embed_src = L.EmbedID(n_voc_src, n_units, ignore_label=-1),
			e_lstm = L.StatelessLSTM(n_units, n_units),
			e_lstm1 = L.StatelessLSTM(n_units, n_units),
			embed_trg = L.EmbedID(n_voc_trg, n_units, ignore_label=-1),
	...
	def __forward(self, train, x_batch, y_limit = 25, y_batch = None, fixed_emb=False):
		...
		for x in x_batch:
			...
			lstm_c, lstm_h = model.e_lstm(lstm_c, lstm_h, F.dropout(x, ratio=0.2, train=train))
			lstm_c1, lstm_h1 = model.e_lstm1(lstm_c1, lstm_h1, F.dropout(lstm_h, ratio=0.2, train=train))
	...
	def translate_beam(self, x_batch, max_length=40, beam=6):
		...
		zeros = mod.zeros((1, n_units), dtype=np.float32)
		lstm_h = Variable(zeros, volatile=not train)
		lstm_c = Variable(zeros, volatile=not train)
		lstm_h1 = Variable(zeros, volatile=not train)
		lstm_c1 = Variable(zeros, volatile=not train)
